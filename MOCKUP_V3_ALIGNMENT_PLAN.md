...
> Plan file copied from `C:\Users\nucif\.claude\plans\ui-wise-finch.md` (2026-05-11) for in-repo reference.
> Status: All phases A-F completed (2026-05-11).
...
| Phase | Status | Notes |
|---|---|---|
| **A** Subtractive Removal | ✅ DONE (2026-05-11) | proto `72d621c`, root `76977ff` |
| **B** Header rebuild (title-bar + menu-bar + ribbon) | ✅ DONE (2026-05-11) | proto `0ec4cd4`, root `614714e` |
| **C** Panels restructure (3 tabs left, layers+props right, status bar) | ✅ DONE (2026-05-11) | same commit as B |
| **D** Summary Widget 4 tabs + drag | ✅ DONE (2026-05-11) | proto `203ae90`, root `2af21a5` |
| **E** CSS palette sync + dead-style cleanup | ✅ DONE (2026-05-11) | `proto/static/css/app.css` 590 → 381 lines (-209). Palette already synced in Phase B; removed `.ulp-*`, `.isp-*`, `.ft-*`, `.wp-*`, `.widget-card/title/body/link/row/badge`, `.widget-hidden`, `.widget-size-*`, `.workflow-card`, `.wf-row`, `.qt-btn`, `#quick-tag-bar`, `#tool-row`, `#float-toolbar`, `#ui-layout-panel`, `#btn-ui-layout`, `.topbar-zone-a/b/c`, `body.ui-{top,left,right,widgets}-v3 *`, `.toolbar-more-wrap`, `#toolbar-more-*`, `.more-section`, `.more-wide`. Trimmed `.ft-btn`/`.ft-label` refs from `@media` queries. |
| **F** Final tests + docs | ✅ DONE (2026-05-11) | smoke + full PASS, all 17 markers OK, no measurement regression. Docs updated: `PATCH_SUMMARY.md`, `TEST_RESULT.md`, `UI_MANUAL_TEST.md`, `FINAL_REPORT_FOR_CHATGPT.md`, `CURRENT_STATUS.md`, `log.md`. |
...
### Phase A — Subtractive Removal ✅ DONE
...
- ⏳ `#tool-row` + `#float-toolbar` (replace ด้วย `.ribbon`) — **deferred to Phase B**
- ⏳ Multi-zone topbar `.topbar-zone-a/b/c` (replace ด้วย title-bar + menu-bar) — **deferred to Phase B**
...
- ✅ Inspection panel: `updateInspectionPanel()`, `toggleInspectionPanel()` stubbed as no-op
- ✅ `updateWidgets()` stubbed as no-op
- ✅ Init calls: ลบ `loadUiLayout(); loadPanelLayout(); loadWidgetPlacement();`

**3. `proto/static/css/app.css` — ลบ CSS (deferred to Phase E):**
...
- ⏳ `toolbarInToolRow`, `toolRowAboveWorkspace` (deferred — will replace with `ribbonAboveMain` in Phase B)

**5. `proto/e2e_ui_test.py` — เพิ่ม assertions ใหม่ (deferred to Phase B-D):**
