...
try:
    _env_upload_mb = int(os.environ.get("BMA_MAX_UPLOAD_MB", "256"))
except ValueError:
    _env_upload_mb = 256
...
# ─────────────────────────────────────────────────────────────
# /export-xlsx-summary — Sprint U2 "1-Page Excel Summary"
# Single-sheet "สรุปผังบริเวณ" workbook for printable 1-page hand-off.
# Client precomputes BCR/OSR/FAR/permeable numbers + setback summary + marker
# counts in `summary` dict; server's job is to render the 1-page XLSX.
# Phase-1 boundary: facts only — NO pass/fail verdict, NO color-coded
# "ผ่าน/ไม่ผ่าน". Limits (projectInfo.userDefinedLimits) are echoed alongside
# the actual values but never compared with a verdict cell.
# ─────────────────────────────────────────────────────────────
@app.post("/export-xlsx-summary")
async def export_xlsx_summary(body: dict):
    if not HAS_XLSX:
        return JSONResponse({"error": "xlsxwriter not installed"}, 501)
    case = _get_case(body.get("case_id", ""))
    if not case: return JSONResponse({"error": "invalid case"}, 400)
    ...
    pdf_name = body.get("pdfName", "export")
    generated_at = body.get("generatedAt") or time.strftime("%Y-%m-%dT%H:%M:%S%z")

    buf = io.BytesIO()
    wb = xlsxwriter.Workbook(buf, {"in_memory": True})
    ws = wb.add_worksheet("สรุปผังบริเวณ")
    # Force 1-page printable
    ws.set_landscape()
    ws.set_paper(9)   # 9 = A4
    ...
    fmt_sub = wb.add_format({"bold": True, "bg_color": "#D6E4F0", "border": 1, "align": "left", "valign": "vcenter"})
    fmt_lbl = wb.add_format({"border": 1, "bg_color": "#F2F6FB", "valign": "top"})
    fmt_val = wb.add_format({"border": 1, "valign": "top"})
    fmt_num = wb.add_format({"border": 1, "num_format": '#,##0.00'})
    fmt_pct = wb.add_format({"border": 1, "num_format": '0.00%'})
    fmt_total = wb.add_format({"bold": True, "bg_color": "#E2EFDA", "border": 1, "num_format": '#,##0.00'})
    fmt_note = wb.add_format({"italic": True, "font_color": "#666666", "border": 1, "valign": "top"})

    row = 0
    # Title row (merged 4 cols)
    ws.merge_range(row, 0, row, 3, f"สรุปผังบริเวณ — {pdf_name}", fmt_title)
    ws.set_row(row, 24)
    row += 1
    ws.write(row, 0, "วันที่ออกรายงาน", fmt_lbl)
    ws.merge_range(row, 1, row, 3, generated_at, fmt_val)
    row += 2

    # ─── Project info block ───
    ws.merge_range(row, 0, row, 3, "ข้อมูลโครงการ", fmt_sub); row += 1
    udl = project_info.get("userDefinedLimits", {}) or {}
    proj_rows = [
        ("เลขที่คำขอ", project_info.get("reqNo", "")),
        ("ประเภทอาคาร", project_info.get("buildingType", "")),
        ("ลักษณะงาน", project_info.get("workType", "")),
        ("จำนวนชั้น", project_info.get("floors", "")),
        ("จำนวนยูนิต", project_info.get("units", "")),
        ("ประเภทการใช้อาคาร", project_info.get("buildingUseType", "")),
        ("รหัสผังเมือง", project_info.get("zoneCode", "")),
        ("ความกว้างถนนเข้าโครงการ (ม.)", project_info.get("siteAccessRoadWidth_m", "")),
        ("GFA เป้าหมาย (ตร.ม.)", project_info.get("gfa", "")),
    ]
    for label, value in proj_rows:
        ws.write(row, 0, label, fmt_lbl)
        ws.merge_range(row, 1, row, 3, value if value not in (None, "") else "—", fmt_val)
        row += 1
    row += 1

    # ─── Land + area block ───
    ws.merge_range(row, 0, row, 3, "พื้นที่ผังบริเวณ", fmt_sub); row += 1
    ws.write(row, 0, "ประเภทพื้นที่", fmt_lbl)
    ws.write(row, 1, "จำนวน", fmt_lbl)
    ws.write(row, 2, "พื้นที่รวม (ตร.ม.)", fmt_lbl)
    ws.write(row, 3, "สัดส่วน (% ของที่ดิน)", fmt_lbl)
    row += 1

    land_area = float(summary.get("landArea") or 0)
    area_rows = summary.get("areaBreakdown") or []
    for ar in area_rows:
        tag_label = ar.get("label") or ar.get("tag") or "—"
        count = ar.get("count") or 0
        area = float(ar.get("area") or 0)
        ratio = (area / land_area) if land_area > 0 else None
        ws.write(row, 0, tag_label, fmt_val)
        ws.write(row, 1, count, fmt_val)
        ws.write(row, 2, area, fmt_num)
        if ratio is None:
            ws.write(row, 3, "—", fmt_val)
        else:
            ws.write(row, 3, ratio, fmt_pct)
        row += 1
    ws.write(row, 0, "พื้นที่ที่ดิน (รวม)", fmt_lbl)
    ws.write(row, 1, summary.get("landCount") or 0, fmt_val)
    ws.write(row, 2, land_area, fmt_total)
    ws.write(row, 3, "—" if land_area <= 0 else 1.0, fmt_pct if land_area > 0 else fmt_val)
    row += 2

    # ─── Ratios block (facts only — no verdict) ───
    ws.merge_range(row, 0, row, 3, "อัตราส่วนหลัก (ค่าจริง — ไม่ระบุผ่าน/ไม่ผ่าน)", fmt_sub); row += 1
    ws.write(row, 0, "BCR (Building Coverage Ratio)", fmt_lbl)
    bcr = summary.get("bcr")
    ws.write(row, 1, bcr if bcr is not None else "—", fmt_pct if bcr is not None else fmt_val)
    ws.write(row, 2, "OSR-min ที่ผู้ใช้ตั้ง", fmt_lbl)
    osr_min = udl.get("osr_min_pct")
    ws.write(row, 3, (osr_min / 100.0) if isinstance(osr_min, (int, float)) else "—",
             fmt_pct if isinstance(osr_min, (int, float)) else fmt_val)
    row += 1
    ws.write(row, 0, "OSR (Open Space Ratio)", fmt_lbl)
    osr = summary.get("osr")
    ws.write(row, 1, osr if osr is not None else "—", fmt_pct if osr is not None else fmt_val)
    ws.write(row, 2, "FAR-max ที่ผู้ใช้ตั้ง", fmt_lbl)
    far_max = udl.get("far_max")
    ws.write(row, 3, far_max if isinstance(far_max, (int, float)) else "—", fmt_num if isinstance(far_max, (int, float)) else fmt_val)
    row += 1
    ws.write(row, 0, "FAR (Floor Area Ratio)", fmt_lbl)
    far = summary.get("far")
    ws.write(row, 1, far if far is not None else "—", fmt_num if far is not None else fmt_val)
    ws.write(row, 2, "พื้นที่ซึมน้ำ-min", fmt_lbl)
    perm_min = udl.get("permeable_min_pct")
    ws.write(row, 3, (perm_min / 100.0) if isinstance(perm_min, (int, float)) else "—",
             fmt_pct if isinstance(perm_min, (int, float)) else fmt_val)
    row += 1
    ws.write(row, 0, "Permeable Area %", fmt_lbl)
    perm = summary.get("permeablePct")
    ws.write(row, 1, perm if perm is not None else "—", fmt_pct if perm is not None else fmt_val)
    ws.merge_range(row, 2, row, 3, "ข้อมูลเชิงข้อเท็จจริงเท่านั้น — ไม่ใช่การพิจารณาทางกฎหมาย", fmt_note)
    row += 2

    # ─── Setback summary block ───
    ws.merge_range(row, 0, row, 3, "ระยะร่นโดยรอบ (เมตร)", fmt_sub); row += 1
    setbacks = summary.get("setbacks") or {}
    sb_pairs = [
        ("ด้านหน้า (front)", setbacks.get("front")),
        ("ด้านหลัง (back)", setbacks.get("back")),
        ("ด้านข้าง 1 (side-1)", setbacks.get("side1")),
        ("ด้านข้าง 2 (side-2)", setbacks.get("side2")),
    ]
    for label, val in sb_pairs:
        ws.write(row, 0, label, fmt_lbl)
        if isinstance(val, (int, float)):
            ws.write(row, 1, val, fmt_num)
        else:
            ws.write(row, 1, "—", fmt_val)
        ws.merge_range(row, 2, row, 3,
                       "ระยะใกล้สุดจาก geometry ที่วัดได้" if isinstance(val, (int, float)) else "ยังไม่ได้วัด",
                       fmt_note)
        row += 1
    row += 1

    # ─── Marker counts block ───
    ws.merge_range(row, 0, row, 3, "หมุดสถานที่", fmt_sub); row += 1
    ws.write(row, 0, "ประเภทหมุด", fmt_lbl)
    ws.write(row, 1, "จำนวน", fmt_lbl)
    ws.merge_range(row, 2, row, 3, "คำอธิบาย", fmt_lbl)
    row += 1
    marker_rows = summary.get("markerBreakdown") or []
    if marker_rows:
        for mr in marker_rows:
            ws.write(row, 0, mr.get("label") or mr.get("type") or "—", fmt_val)
            ws.write(row, 1, int(mr.get("count") or 0), fmt_val)
            ws.merge_range(row, 2, row, 3, mr.get("note") or "", fmt_val)
            row += 1
    else:
        ws.merge_range(row, 0, row, 3, "ยังไม่ได้วางหมุด", fmt_note); row += 1

    # ─── Footer note ───
    row += 1
    ws.merge_range(row, 0, row, 3,
                   f"BMA-Plan Phase 1 · สรุป 1 หน้า · {generated_at} · ไม่มีการพิจารณาผ่าน/ไม่ผ่านตามกฎหมาย",
                   fmt_note)
    ...
    return Response(buf.getvalue(),
                    ...
                    headers={"Content-Disposition": f'attachment; filename="{safe}_summary.xlsx"',
                             "X-Bma-Summary-Mode": "1-page"})
