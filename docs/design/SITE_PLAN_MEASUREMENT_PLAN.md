...
> Date: 2026-05-13 (reconciled 2026-05-14 — added `fire_escape` / `fire_elevator` markers + §10.4 classification hierarchy)
...
| บันไดหนีไฟ / ทางหนีไฟ | ❌ ต้องเพิ่ม | มร.33 (ระบบหนีไฟ) — มักอยู่ plan/section page |
| ลิฟต์ดับเพลิง | ❌ ต้องเพิ่ม | มร.33 (ลิฟต์ดับเพลิง) — มักอยู่ plan/section page |

→ เพิ่ม `markerType` enum: `parking | parking_disabled | parking_fire | parking_ambulance | entrance | aed | sign | tree | fire_escape | fire_elevator`

> **Note (reconcile 2026-05-14):** `fire_escape` / `fire_elevator` มาจาก user model — โดยธรรมชาติเป็น object บน plan/section page ไม่ใช่ผังบริเวณ แต่ `markerType` เป็น generic enum ใช้ได้ทุก `pageType` จึงเพิ่มได้โดยไม่กระทบ site-plan scope
...
### 10.4 Classification Hierarchy — "วาดแล้วค่อยเลือกว่าเป็นอะไร" (UX framing, reconcile 2026-05-14)

โมเดลที่ผู้ใช้ยืนยัน: เครื่องมือพื้นที่มีแค่ **2 วิธีวาด** (`polygon` / `polygon+curve` — Phase H.1 ทำแล้ว) ส่วน "เป็นอะไร" เลือกหลังวาดผ่าน Properties panel — ไม่ใช่ tool-per-type. การเลือกเป็นลำดับชั้น ซึ่ง map ตรงกับ field pyramid ที่มีอยู่:

| ระดับที่ผู้ใช้เห็น | field ที่มีอยู่ | ตัวอย่าง |
|---|---|---|
| 1. ใช้เป็นอะไร "ตามแบบ" | `measurementProfile` / `objectCategory` | ห้อง, ทางเดิน, ที่จอดรถ, พื้นที่ปกคลุม |
| 1.1 ใช้เป็นอะไร "ตามกฎหมาย" | `reportTarget` / `lawBasis` | BCR, OSR, GFA, ที่ว่างตามกฎหมาย |
| (พิเศษ) ช่องว่าง / หักลบ | semanticTag `deduction_opening` | ช่องเปิด, void |

→ **ไม่ใช่ schema ใหม่** — เป็นการ *จัดระเบียบ UI* ของ field ที่มีอยู่แล้ว (`semanticTag` → `measurementProfile`/`objectCategory`/`reportTarget`/`lawBasis`). งาน UI hierarchy นี้อยู่ **Phase I-B** (Site Plan tools / Properties panel) — ไม่ใช่ Phase I-A. Phase I-A เพิ่มแค่ enum value + Project Setup fields เท่านั้น.
...
## 16. Open Questions (ต้อง decide ก่อน implement) — **DECIDED 2026-05-13**

> Decisions confirmed by product owner (user) 2026-05-13.
> Rationale: lowest-risk, Phase 1 strict (no Rule Engine), additive-only schema, defer cross-object linking to later sprints.

### Q1: User-defined limits vs Reference table — **A**
- **A. ผู้ใช้กรอกเองทั้งหมด (Phase 1 strict)** ✅ **DECIDED**
- B. มี dropdown โซนผังเมือง (ใส่ FAR/OSR auto) — เริ่มกลายเป็น Rule Engine
- C. Mix: เริ่ม A, ภายหลังเพิ่ม B เป็น Phase 2

Rationale: ตรงกับ Phase 1 hard rule (§2, §15) — no Rule Engine, no auto-judgment. ผู้ใช้ป้อน limits เอง, ระบบไม่ตัดสิน.

### Q2: Building object บน site plan — **A**
- **A. เพิ่ม polygon `building_coverage` ปกติ + `buildingHeight_m` field** ✅ **DECIDED**
...
Rationale: schema-additive น้อยที่สุด, ไม่ต้องสร้าง object type ใหม่ — ตรงกับ Phase I-A scope (lowest risk).

### Q3: Multiple buildings (มร.55 ข้อ 48) — **A**
- **A. เก็บแยกเป็น polygon แต่ละหลัง** ✅ **DECIDED**
- B. Group หลายหลังเป็น 1 polygon รวม

Rationale: แยก polygon รองรับการคำนวณ setback/2h-rule ต่อหลังได้, เป็นทางที่ §16 เดิมระบุว่า "recommended".

### Q4: 2h rule (มร.55 ข้อ 44) — **DEFERRED**
- **Defer ออกจาก Phase I-A** ✅ **DECIDED**
- ต้องการ Phase H.0 (45° angle lock) implement ก่อน
- เมื่อพร้อม: ระบบวัดได้ — ผู้ใช้เปรียบเทียบเอง (no auto-verdict)

Rationale: 2h rule ต้องการ Phase H.0 prerequisite ตามที่ระบุไว้เดิม. Phase I-A จะไม่แตะ 2h rule.

### Q5: ที่จอดรถ — area polygon vs marker — **A + B (ไม่ link)**
- **A. Marker (point) สำหรับนับจำนวนคัน** ✅ **DECIDED — รวมใน I-A/I-B**
- **B. Polygon `parking_area_outdoor` สำหรับขนาดพื้นที่** ✅ **DECIDED — รวมใน I-A/I-B**
- C. ทั้ง 2 — marker linked to polygon (auto count per polygon) — **DEFERRED ไปสปรินต์ถัดไป (I-C หรือหลัง)**

Rationale: A + B แยกกันเป็น additive schema ปกติ. การ link 2 objects เข้าด้วยกัน (auto-count per polygon) เป็น cross-object relationship ที่เพิ่ม complexity — เลื่อนไปสปรินต์ถัดไป.

---

### Summary of decisions — impact on Phase I-A scope

| Q | Decision | Phase I-A includes? |
|---|----------|---------------------|
| Q1 | A — user-defined limits | ✅ Project Setup fields: `userDefinedLimits` (BCR/OSR/FAR/setback per direction), `zoneCode` (string label only, no lookup) |
| Q2 | A — polygon + height field | ✅ Add `buildingHeight_m` to building_coverage object schema |
| Q3 | A — separate polygons per building | ✅ Each `building_coverage` polygon is independent — no grouping field |
| Q4 | Defer | ❌ Not in I-A — wait for Phase H.0 |
| Q5 | A + B separate | ✅ Add marker types + `parking_area_outdoor` polygon tag — no link field |

**Phase I-A is now unblocked.** Next sprint card: `RUN_PHASE_I_A_SCHEMA_AND_PROJECT_SETUP.md` (to be created).
