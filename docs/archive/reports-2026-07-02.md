...
- **(1) `BUG-20260702-lite-pagerot-registration`** — manual page rotate desyncs stored geometry from the raster. **SHIPPED same-day.**
...
# Archived: BUG-20260702-lite-pagerot-registration — PASS
...
# Archived: PERF-20260702-lite-foxit-smoothness — PASS
...
- **(5) `ptToScreen`/`screenToPt` parity-fixture addition** — remains queued from the pagerot-registration sprint. **(Closed with rationale by `GO-20260703-invariants-streaming-worker-recycle`'s V2-U1 invariant registry — registered as invariant I7 instead.)**
