...
- [ ] **Focus-mode lite-version spinoff with single-row menu bar** — `invent-queued` — from /idea 2026-05-20 (user enjoys F11 / Focus mode in current app — wants to know if a standalone "ver lite" with only a one-row menu bar is feasible; also requests competitive research on construction area-measurement software and how they handle large drawings, with results delivered as an idea card)
    - Source: user 2026-05-20, "แตกประเด็น ชอบ ui แบบ f11 และ Focus mode ถ้าจะแตก ออกจากโปรแกรมหลัก มาทำ ver lite โดย มีเมนูบาร์แค่แถวเดียว..."
    - Tags: bma-plan, ui, focus-mode, fullscreen, lite-version, research-needed, p-med
    - Direction: (unframed — pending /bma-invent FRAME phase; possible spinoff branch vs. enhanced focus-mode-in-place)
    - Open questions: (pending /bma-invent — user explicitly requested construction-measurement competitor research + large-drawing handling — will run in Phase 2 via `bma-researcher`)
    - Scope skill: pending (`/bma-invent` decides after research; likely `/bma-ui-scope` if in-place, or new packaging-scope if true spinoff)
    - Forbidden-surface profile: unknown — `/bma-invent` checks during RESEARCH (spinoff path may touch packaging/launch, not measurement math)
