...
- [x] **LCAL-S5** ✅ DONE 2026-05-23 — width refinement + sandbox align. `lite/static/css/layer-panel.css`: `body.lt-v2 #picker { width: 320px → 440px }` (+2 lines for inline comment). Sandbox spike `lite/sandbox/invent-layer-ui-spec.html` synced to live state: v2 width 440, added 🎨 button mock in v1 header (matches LCAL-S3 ship), annotation table updated. User requested after seeing 4-width comparison; 440 picked for breathing room w/o overeating canvas. Verified 440px in both live + spike via Playwright.
