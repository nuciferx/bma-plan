...
|---|---|---|---|---|---|
| HT-ACC-1 | **FRICTION (accuracy-critical)** — calibration snap silently latches both points to the WRONG nearby vector line (human-test: ~400pt click → snapped to 1663pt line, 4× misfire) with NO visual confirmation of which segment was captured. If the snapped line is ~1% longer than intended → `pts_per_m` ~1% high → **all areas ~2% too small, systematically** (matches user report: โฉนด 2-2-0=4000m² measures smaller). Fix (UX only, NO area-math touch): after the 2nd calib click, flash/highlight the snapped segment on canvas + show "ตรึงที่เส้น X pt — ใช่เส้นที่ต้องการ?" with confirm/retry; consider a "no-snap precise" toggle for calibration. scope=`/bma-measure-ux` · regression=`/bma-measure-regression` · marker=`HT_ACC1_CALIB_SNAP_OK` · source=human-test 2026-05-20 | `/bma-measure-ux` | measure-ux (calibration) | queued | — |
| HT-ACC-2 | **FRICTION** — calibration accuracy guidance bundle: (a) calib panel tip "ใช้เส้นอ้างอิงที่ยาวที่สุดที่ทราบระยะ" (longest baseline = best accuracy); (b) post-calibration nudge + promote **Verify Scale** to a ribbon button beside Set Scale (currently buried in Scale menu — users won't find it); (c) when drawing area on a `site`/`land` page with long edges, hint "ขอบโค้งให้ใช้เครื่องมือ arc เพื่อความแม่นยำ" (straight chords under-report curved boundaries). scope=`/bma-ui-ribbon` + `/bma-measure-ux` · marker=`HT_ACC2_CALIB_GUIDE_OK` · source=human-test 2026-05-20 | `/bma-ui-ribbon` + `/bma-measure-ux` | ribbon + measure-ux | queued | — |
| HT-ACC-3 | **COSMETIC** — scale badge shows rounded `1:17` while actual is `1:17.042` (0.25% label-only; area uses exact float, no measurement impact). Show one decimal `1:17.0` and the exact `pts_per_m` in the scale-panel tooltip for expert cross-check. scope=`/bma-ui-status` · marker=`HT_ACC3_SCALE_DECIMAL_OK` · source=human-test 2026-05-20 | `/bma-ui-status` | status-bar / canvas-ui | queued | — |
| HT-NAV-1 | **COSMETIC/investigate** — on the 11-page raster PDF `ข.4`, the journey script's page-next loop exited after page 1 (btn-next threw a silently-caught exception); pages 2–11 not auto-measured. Likely a test-script artifact, but verify real next-page navigation works on multi-page raster PDFs (no vector geometry). scope=`/bma-ui-ribbon` (nav buttons) · source=human-test 2026-05-20 | `/bma-ui-ribbon` | navigation | queued | — |
