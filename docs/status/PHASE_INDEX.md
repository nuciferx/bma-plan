...
**Update 2026-05-18 evening:** `INV-2026-05-18-001 Page Setup Redesign` added to queue after `/bma-invent` GO checkpoint, then split into 001a/b/c by `/bma-dev-loop` SCOPE step. **001a shipped** as repo's initial commit `e85a5ce` — context-sensitive inspector + traffic-light chips; smoke `PHASE_INV_PAGE_SETUP_A_OK 8/8 PASS`. 001b/c remain queued.
...
| INV-2026-05-18-001c | **Page Setup Redesign Part C — Permanent delete with renumber-map + `/rebuild-pdf` server endpoint** — Activate Danger Zone in page-card → renumber-map preview dialog (old → new mapping, deleted row crossed-out) → confirm calls new `/rebuild-pdf` POST endpoint. **Research done 2026-05-19** (PRIOR_ART_PARTIAL — Bluebeam/Foxit/Adobe all do implicit renumber + session undo; BMA's preview is genuinely better UX). **Q1–Q4 design answers locked**: Q1 = ADD new `/rebuild-pdf` (do not edit existing core endpoints); Q2 = in-place `CASES[case_id]["doc"]` + flush caches + bump `docVersion`; Q3 = server-authoritative renumber map → client `_reindexPageDicts(renumberMap)` walks 7 per-page dicts; Q4 = hard-block during draw (HT-7 pattern). Server: PyMuPDF `doc.delete_page()` reverse order, in-place, re-analyse. Confirmation = renumber preview + "ลบไม่ย้อนกลับได้หลังบันทึก" warning (Foxit style). Undo session-scoped via `pushUndo()` before commit. Schema additive: `deletedPageNumbers: number[]` (audit only, cleared after rebuild). Marker: `PHASE_INV_PAGE_SETUP_C_OK` 9 sub-checks. | `/bma-ui-scope` + `/bma-check-forbidden` (server endpoint addition pre-approved per research) | UI / delete + server / `/rebuild-pdf` | queued (research-armed) | INV-2026-05-18-001a |
