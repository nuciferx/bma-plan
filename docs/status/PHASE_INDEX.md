...
| INV-2026-05-20-004 | **Layer rebuild L3 — render reads page-scoped + delete legacy** (invent-done-go). Switch `redraw()` (ui.html:1905) + opening/visibility filters to read the object's page-scoped layer (`layerId`→layer.visible/locked) instead of `areaTypeLayer(poly.areaType)`+global `layerVis`. Delete the global `areaTypeLayer` render path + global `layerVis`/`layerLock` once nothing reads them (single system remains). Fixes the visible overlap bug (7 site tags no longer share one render bucket; hide/lock/solo per layer works). Forbidden: NONE area-math; render-only. Scope doc audit (render tail). Markers `INV_LAYER_L3_OK` + existing layer/render markers stay GREEN. Requires `/bma-ui-regression`. | `/bma-ui-canvas` + `/bma-ui-regression` | ui-canvas + render | queued | INV-2026-05-20-003 |
