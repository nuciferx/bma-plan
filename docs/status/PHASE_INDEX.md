...
| INV-2026-07-04-002 | **Page tagging rework: JIT + range + verify grid + fearless retag — lite (approach C+minimal-A, GO 2026-07-04)**. Demote tag-all-upfront wizard to optional tool; spike 3/3 (7/8 actions happy path), opus CONFIRM-WITH-CONCERNS, user GO. **Slice order + acceptance criteria:** (1) **extract `overview-grid.js` FIRST** — overview-setup.js 1059 > cap 1000, closure-coupled region (`_lovsSelected`/bulkbar/tiles), no feature lands before this; (2) bulk-bar upgrade: tag+kind+start# apply with auto-increment, mezzanine no-num skip, **basement นับถอยหลังตาม `_lovsSequentialFloor` จริง (overview-setup.js:597-615) — spike expected map นับขึ้น ห้าม copy**; overwrite-confirm (นับ overlap, cancel = zero-op); single `pushUndo()` ครอบทั้ง batch; (3) view toggle เรียงหน้า↔จัดกลุ่มตามแท็ก (ระวังชน `ltDndDecorate` wiring บน tiles เดิม — riskiest per inventor); (4) JIT banner: วัดบนหน้าไม่แท็ก → refuse+prompt 1-tap (per-page hard gate, HT-7); **ถอด SET gate `_lovsAnyTagged` (overview-setup.js:284-289) พร้อมกันเท่านั้น** — ห้ามมีช่วงที่ไม่มี gate เลย; (5) **integration proof บน remap engine จริง** — retag หน้าที่มี objects จริงต้องผ่าน `_pflFolderHasUserDrawnObjects` guard + `reseedActivePageFolders` + `_docSnap` undo (spike ใช้โค้ดจริง 0 บรรทัด — ข้อนี้คือความเสี่ยงหลัก) + เทสกับ permit 45 หน้า. Schema: additive-only (คาดว่าไม่ต้องแตะ). Doc: `docs/invent/lite-page-tagging-workflow.md` · Spike: `lite/sandbox/invent-lite-page-tagging-workflow.html` (+ `-eval.py`). Forbidden: NONE. **✅ SHIPPED 2026-07-04 — 4 slices:** (1) extract `overview-grid.js` (overview-setup 1059→850, under cap); (2) bulk apply: auto-number + basement descending (cross-verified vs `_lovsSequentialFloor`) + overwrite-confirm + single-undo (`LITE_BULK_APPLY_OK` 5/5); (3) view toggle จัดกลุ่มตามแท็ก + Fable drift-guard (unknown tags never hidden) (`LITE_GRID_GROUP_VIEW_OK` 5/5); (4) `tag-jit.js`: per-page JIT gate via single `setTool` wrap + SET gate removed atomically + keyboard visual-order fix + **real-engine retag zero-loss proof** (`LITE_TAG_JIT_OK` 6/6 incl. guard+reseed+undo on real writers). Absorbs REVIEW ledger **S-10**. Documented semantics: per-page kind wins over bulk selector; stale floorNum not cleared on kind change (harmless via liteSetFloorKind delete branch). | `/bma-lite-dev` | lite / page-setup workflow | ✅ shipped (pending user field test) | INV-2026-07-04-001 ✅ |
| INV-2026-07-04-001 | **Layer panel: context-scoped + floor-rail + search — lite (approach B, GO 2026-07-04)**. Rethink for 100-floor scale: panel renders ONLY the active floor's `PF_*` folder; floor-rail (prev/next + jump dropdown, kind-aware order B3..B1,1..12,M,13..ROOF); global search (floor label + layer name) that jumps + highlights. Spike passed eval 9/9 (3 cases × 3 proportion presets), opus first-stage review CONFIRM-WITH-CONCERNS, Fable+user GO. **Proportions locked: P2 Balanced 260px** (30px row rhythm + chips; token values in spike `--p-*` vars). **Acceptance criteria (from checkpoint):** (1) **ผูกหน้า PDF สองทาง** — เปลี่ยนหน้า canvas → panel สลับ folder อัตโนมัติ AND กด rail/search → canvas วาร์ปไปหน้าที่ผูกไว้ผ่าน `PF_*.pages` (**subsumes LFOC-1e**); (2) search-cap UX — cap 30 ผลลัพธ์ต้องมี count badge / group-by-floor (spike ไม่ได้ gate เคสชื่อ layer ซ้ำ 100 ชั้น); (3) integration จริงเข้า `layer-panel.js`/`layer-tree.js` (spike เป็น clean-room — 25-node win พิสูจน์เฉพาะ isolation) + เทสกับ permit 45 หน้าจริง; (4) v1 `.bmaplan` โหลดได้ lossless (zero schema change by design — global-monotonic `layer.order` convention ต้องคงไว้). Phase-2 candidate (แยก sprint): approach E visibility presets ("โครงสร้างทั้งตึก" กดทีเดียว). Doc: `docs/invent/lite-layer-menu-ui-fix.md` · Spike: `lite/sandbox/invent-lite-layer-menu-ui-fix.html` (+ `-eval.py`). Forbidden: NONE. **Shipped `7600fde` 2026-07-04** (3 slices: layer-scope.js new 531L + rail/search/P2-260px; 10 test suites green; real-permit proof 260px/0 errors). **Slice 4 shipped `e1c6a76`:** user field bug (กลับหน้าเดิมเห็น layer เดิม) — root cause = global `state.activeCat` ไม่มี per-floor memory (ไม่ใช่ rendering bug; repro 3 เส้นทางยืนยันแถวไม่หาย); fix = `_lsSyncActiveCatToFolder` per-folder active memory, runtime-only. ⚠️ awaiting user field re-test — ถ้าอาการยังอยู่ต้องขอ step-by-step repro. Known follow-ups: test_undo_layers.py cp1252 print crash (pre-existing, housekeeping); dead tree-render path → LFOC-1b. | `/bma-lite-dev` | lite / layer-ui | ✅ shipped `7600fde`+`e1c6a76` (pending user field re-test) | — |
