...
| INV-2026-05-18-002 | **Settings/Preferences panel v2 — Export Defaults + Loupe Prefs** — Extends `bmaPlan.settings.v1` with 4 new PREFS paths (additive). `export.csvSeparator` (`,`/`;`/`\t`) + `export.includeLawBasis` slot into **หน่วย** tab. `loupe.radius` (50–160) + `loupe.zoomFactor` (2–8×) slot into **วาด** tab. Callsite wiring: `exportCSV` reads both export prefs (conditionally appends `lawBasis` column, dynamic separator + escape regex); `updateLoupe` uses `loupeZoomFactor` for constant magnification (SW = loupeR×2/zoomFactor). New `_applyLoupePrefs()` syncs state vars on boot + after Apply (clamps before fallback so 0→2 not 4). `SETTINGS_V2_OK` 6/6 PASS. Zero forbidden-surface. Zero `.bmaplan` schema change. v1 untouched (`SETTINGS_OK` still GREEN). Source: `docs/invent/settings-panel-v2.md` + spike `proto/sandbox/invent-settings-panel-v2.html`. | `/bma-ui-scope` (modal region) | UI / settings modal | ✅ done | INV-002 ✅ |
