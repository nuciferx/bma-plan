...
| INV-2026-05-24-002a | **Centerline snap for dashed boundary lines — proto** (invent-done-go). 2-step hybrid (per-click ROI snap + post-draw per-edge PCA + corner intersection) ที่ pin user click ไปยังกึ่งกลาง stroke ของเส้นปะหนา → ขอบนอก/ขอบใน/กลาง วัดได้ค่าเดียวกัน (spike ±0.185%, ภายในเป้า ±0.5%). NEW `proto/static/js/centerline-snap.js` (~150 LOC) — inline Zhang-Suen, ไม่ใช้ CDN. Hook ใน area-tool click handler ใน `proto/ui.html` (~10 LOC). Toggle ใน Helpers ribbon (sibling Loupe/Ortho/Perp/Snap-off จาก HT-13a). Live mode = step 1 only (per-mousemove ถ้า toggle ON); commit = step 1 + step 2 (post-Enter ก่อน `pushUndo()`). Schema additive: optional `obj.traceMode="centerline-roi"`; save/load round-trip; absence = legacy raw click. Threshold = Otsu (adaptive). Marker `PHASE_CENTERLINE_SNAP_OK` ≥ 8 sub-checks (lib loaded / fn exists / ROI snap correct / refine correct / schema additive / toggle persists / no PATH_GEOMETRY_OK regression / no ARC_POLYGON_OK regression / synthetic + 1 real-raster sample). **Forbidden: NONE** (no polyAreaM2/polyMetrics/polySelfIntersects/pdfToC/cToPdf/RS/snap-internals touched; uses `getImageData` ของ canvas API). Spike: `proto/sandbox/invent-centerline-snap-dashed-boundary.html` (open in browser, `?auto=1` runs test) — PASS 4/4. Verifier: `artifacts/invent/centerline-snap-dashed-boundary/verify_spike.py`. Source: `docs/invent/centerline-snap-dashed-boundary.md`. User GO 2026-05-24. | `/bma-measure-scope` (sub-area: ux + geometry) → `/bma-measure-ux` + `/bma-measure-geometry` → `/bma-measure-regression` | measure / new helper module + ux + schema additive | 🟡 queued | — |
| INV-2026-05-24-002b | **Centerline snap for dashed boundary lines — lite** (invent-done-go). Vendor 002a's `centerline-snap.js` เข้า `lite/static/js/centerline-snap.js` (same algorithm, same API). Hook ใน area-tool click ของ `lite/ui-lite.html`. UI toggle ใน lite Measure menu / Helpers section. Marker `LITE_CENTERLINE_SNAP_OK` parallel กับ proto. ตาม lite-vendoring contract — math = drift-locked กับ proto's commit. **Forbidden:** ไม่แตะ `lite/static/js/measure-engine.js` (vendored math), `RS`, `pdfToC`/`cToPdf` ของ lite. Hook scope = click handler + toggle UI เท่านั้น. Use `/bma-lite-dev` (one slice). | `/bma-lite-dev` | lite / new helper module + ux | 🟡 queued | INV-2026-05-24-002a |
