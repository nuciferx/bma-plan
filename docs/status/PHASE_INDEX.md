...
| INV-2026-05-18-001c | **Page Setup Redesign Part C — Permanent delete + renumber-map + `/rebuild-pdf`** — ✅ shipped 2026-05-19. Server: new `/rebuild-pdf` POST endpoint (`proto/server.py` ~line 715) — PyMuPDF `doc.delete_page()` reverse order, in-place, flush `page_cache`/`image_cache`, returns `{totalPages, renumberMap, deletedNumbers}`. Client: `_openRenumberDialog(n)` shows old→new preview table (deleted row crossed-out, "ลบ" in red); confirms call `/rebuild-pdf` → `_reindexPageDicts(renumberMap)` walks 7 per-page dicts (`pageStore`/`pageTags`/`pageNames`/`pageRotations`/`excludedPages`/`pageFloorKind`/`pageFloorNum`); `docVersion++` invalidates thumbnail URLs; redirects `curPage` if deleted. Hard-block during draw (`mPts.length > 0` refuses). Last-page guard (refuses if would leave <1 page). Session-scoped undo via `pushUndo()`. Foxit-style warning "ลบไม่ย้อนกลับได้หลังบันทึก" in dialog. `PHASE_INV_PAGE_SETUP_C_OK` 7/7 PASS. | `/bma-ui-scope` + `/bma-check-forbidden` | UI / delete + server / `/rebuild-pdf` | ✅ done | INV-2026-05-18-001a |
