...
|---|---|---|---|---|---|
| PERF-20260702-open-streaming | **Open-perf (lite) — first-paint + memory gap vs Foxit** — measured 2026-07-02 (`artifacts/perf/probe_results_20260702.txt`): RAMA4 18.3MB/45pg first-paint **3,933 ms** cold (upload 2,745 + full-buffer + render); **CHH 90.8MB/95pg (real customer): first-paint 9,639 ms, mem after visiting 10 pages = 765.8 MB(!)**; small 0.7MB file still 1,263 ms = fixed ~1.2s PDF.js-boot+render overhead. Foxit-class target <1s. Zoom re-render 41-71 ms on big files (excellent — keep). Page switch 250-830 ms. **Time attribution (final probe): upload DOMINATES** (~80 ms/MB — server receive + temp write + fitz open; CHH 7.5 s of the 9.6 s), `/raw` full fetch nearly free on localhost (91 MB = 498 ms), PDF.js parse+first-render a flat ~1.2-2 s tail regardless of size. Fix options RE-RANKED accordingly: (1) **LRU eviction on pageCache/PDFPageProxy** ✅ done 2026-07-02 (`test_pagecache_lru.py` LITE_PAGECACHE_LRU_OK, MAX_PAGE_CACHE=4) — cache bounded 12→4 keys, CHH heap 766→~628 MB (-18%); remaining ~600 MB is PDF.js DOC-level shared objects (fonts/XObjects) → owned by option (4) streaming invent, NOT page eviction; (2) **local-file direct open** ✅ done `3ec9239` (`test_local_open.py` LITE_LOCAL_OPEN_OK) — openLocal/adoptCase: paint at 475 ms with upload still in flight, ZERO /raw fetches, server-gated features safe during window, upload-first fallback kept; (3) worker warm-up ✅ done `e0fb856` (`__pdfjsWarmed` at app idle); (4) Range-request streaming [invent — **SPIKE DONE 2026-07-03 (`9466fe4`): NOGO as memory fix** — streaming fetched only 20% of bytes on CHH yet RSS dropped just 10% (1675→1503 MB); bug #10730 CONFIRMED on 4.0.379 (worker heap survives destroy); the ~1.5 GB ceiling is the pdf.js WORKER heap. Bonuses: non-linearized PDFs stream fine + Starlette /raw already 206-ready. **RESHAPE worker-recycle ✅ BUILT** (GO received via /loop 2026-07-03): explicit PDFWorker ownership + _docSource re-open handle (local blob = zero-network reinit) + recycleDocWorker (idle ≥5 min & >20pp / hidden ≥60 s / recycleNow) + transparent lazy reinit; guard `LITE_WORKER_RECYCLE_OK` 7 checks incl. zero-refetch + heap-not-worse; **production RSS re-probe on CHH ✅ PASS 2026-07-03** (`artifacts/perf/probe_worker_recycle_chh_20260703.txt`): recycleNow() released **~1,444 MB — tree RSS 1,796→352 MB (−80.4%)**, renderer −87.7%, jsHeap 541→3.7 MB; ยิ่งกว่า bar ≥50% ชัดเจน; caveat = OS reclaim ช้า (วัดที่ +7s เห็นแค่ −28%, เสถียรที่ +17s) — monitoring ที่ sample <10s หลัง recycle จะเห็นต่ำกว่าจริง; reinit โปร่งใสจริง (page 12 หลัง recycle = 1,138 ms จาก local Blob, zero network; steady-state 584 MB ไม่บวมกลับ). **Streaming-as-bandwidth card: DEFERRED-until-remote-deployment with rationale (เสา 1)** — bytes are localhost on the desktop product (91 MB /raw = 498 ms measured); the 80% byte saving buys nothing until a remote deployment exists. Invent doc: `docs/invent/lite-range-streaming.md`]; (5) adjacent-page prefetch ✅ done `e0fb856` (`test_warm_prefetch.py` LITE_WARM_PREFETCH_OK — n±1 at idle, LRU bound holds); (6) thumbnail warm ✅ done `a0c1152` (`test_thumb_warm.py` LITE_THUMB_WARM_OK — sequential 120 ms gaps, no parallel burst per lpm-8 lesson, no concurrent fitz pressure per S2, token-cancelled). **Foxit-parity status: open ~0.5-1.5 s any size ✅, page switch perceived-instant ✅, zoom/pan at par ✅ (41-71 ms, pan-blank refuted), overview hot ✅, memory page-level bounded ✅ / doc-level → (4) invent.** NOTE placeholder-JPEG idea REJECTED by data: `/page/1` needs upload done first, so it cannot mask the dominant cost. **Pan-blank REFUTED** (0/10 blank across 3 files × 2 passes, zero console errors) → render-followups (b) closed-no-bug. **Overview thumbs RESOLVED — probe artifact** (openOv wrapped by overview-setup renders into #grid-classify not #ov-grid): real numbers 45/45 thumbs, 9.2 s cold (~200 ms/thumb server render), instant warm. source=perf probe 2026-07-02 | n/a (lite render/server) | lite / perf | 🟡 invent-queued | AUDIT-20260702-render-followups |
