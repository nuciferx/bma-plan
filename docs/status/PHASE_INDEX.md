...
| INV-2026-05-19-001a | **Zen Mode + Sheet Minimap** — body.zen CSS class hides ribbon/lp/rp/status (grid-template-rows override = canvas ≈ 96.7% vh on 1080p). 3 corner HUDs replace status bar: TL (Scale + Tool), TR (Page + Exit Zen chip), BL (Layer + Save + Obj + ⚠). #zen-minimap 240×170 bottom-right with 5-col grid of 45 thumbnail cells, IntersectionObserver lazy-load (no concurrent /thumb fetch — protects against malloc anti-pattern). F11 toggle + Esc exit + visible chip. `PREFS.layout.zenMode` additive boolean. Onboarding toast on first zen entry (auto-dismiss). HT-10 `applyLayoutPrefs()` restores prior panel state on zen exit. `PHASE_INV_ZEN_OK` ~10 sub-checks (toggle, canvas≥92%vh, HUD content, minimap lazy-load count, F11/Esc/chip exit, HT-7 scale-gate survival, PREFS round-trip, modal positioning in zen, prefer-classic-on-load default). Est ~180 LOC. Zero forbidden-surface. Schema additive. Source: `docs/invent/fullscreen-canvas-ui.md` (Approach A, 27/30) + spike `proto/sandbox/invent-fullscreen-canvas-ui.html` (6/6 PASS). User GO 2026-05-19. | `/bma-ui-scope` → `/bma-ui-canvas` + `/bma-ui-status` | UI / canvas overlay + HUDs + new zen-mode surface | queued | HT-10 ✅ |
| INV-2026-05-19-001b | **⌘K Command Palette (page jump)** — `#cmd-palette` floating modal (400×320). Input filters pages by number, name, tag (site/plan/elev/section/detail/sys), discipline label. Pre-filtered to 12 default rows on empty input. ↑↓ keyboard nav, Enter = `loadPage(n)` + close, Esc = cancel. Bound to `Ctrl+K`/`Cmd+K` keydown with `!isDrawing` guard to avoid mid-polygon hijack. Reads `pageNames`/`pageTags` from existing pageStore. Works in both classic and zen mode (composes cleanly with 001a). Independent of 001a — can ship before/parallel/after. `PHASE_INV_PALETTE_OK` ~8 sub-checks (open via Ctrl+K, fuzzy filter, ↑↓ nav, Enter jumps, Esc closes, mid-draw guard, classic+zen co-existence, pre-filter default). Est ~130 LOC. Zero forbidden-surface. Schema unchanged (purely UI). Source: `docs/invent/fullscreen-canvas-ui.md` (Approach B, 28/30) + spike same file. User GO 2026-05-19. | `/bma-ui-scope` → `/bma-ui-menu` (palette as modal-like surface) | UI / modal + keyboard | queued | — |
