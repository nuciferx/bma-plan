...
> **Done rows live in [`ROADMAP_DONE.md`](ROADMAP_DONE.md); append-only.** This file keeps only `queued` / in-flight / superseded / deferred rows.
...
|---|---|---|---|---|---|
| UX-20260703-review-findings | **UI/UX review bundle (lite)** — from journey review (artifacts/ux-review-20260703/, 29 screenshots) + static inventory 2026-07-03. CRASH#1 (save wipe) fixed separately. Remaining, ranked: **FRICTION** F-1 `⇧D` Path hotkey DEAD (advertised in flyout+cheatsheet, no keydown handler) · F-2 `F` key=Fit ชนปุ่ม "Focus F" (Focus ไม่มีคีย์จริง) · F-3 Page Manager ทั้งฟีเจอร์เข้าได้แค่ ⇧F12 ไม่มีเมนู (`window.pmOpenManager` exposed แต่ไม่มีใครเรียก) · F-4 สถานะ scale verified มองไม่เห็นหลังปิด modal (HUD โชว์ ✓ เหมือนกันหมด — ทำลาย point ของ Verify) · F-5 wizard block เมาส์เงียบ (hint เฉพาะ keydown, `wiz-auto.js:73`) · F-6 alert "เปิด PDF ก่อน" ระหว่าง background upload ทั้งที่ PDF เปิดอยู่ · F-7 **modalOpen() ไม่ครอบ #sum/#vs-modal/cheatsheet → พิมพ์ชื่อชั้นใน Summary แล้วตัวอักษรสลับเครื่องมือ** · F-8 error ดิบไม่มี next step · F-9 verify ใช้ window.prompt ต่างจาก Set Scale modal · summary เปิดมาเจอ 3 error แดง (seed FAR/OSR ก่อนมีข้อมูล — ชน Phase-1 scope ด้วย) · wizard Next ผ่านได้ที่ 0/45 tagged · export entry ซ้ำซ้อนหลายทาง **COSMETIC**: annotate 7 เครื่องมือไม่มี hotkey เลย · Thai/English สลับกลางเมนู (PM/wizard อังกฤษล้วน) · cheatsheet ตกหล่น 5 รายการ+ผิด 2 · empty state โชว์ layer panel เต็มก่อนเปิดไฟล์ · scanned/fallback ไม่มี badge ถาวร **GOOD (ห้าม regress)**: setup wizard + local-first status + live measurement feedback. **Batch 1 ✅ `34aefa3`** (F-7 modalOpen guard / F-1 ⇧D / F-2 F-key / F-3 PM menu + cheatsheet, `LITE_UX_QUICKWINS_OK`). **Batch 2 ✅ `036a49d` 2026-07-03** (F-4 HUD ✓✓ ยืนยันแล้ว / F-5 mousedown hint / F-6 gateNoCaseMsg upload message / F-9 verify in-app modal / seeded-vars → dim "รอข้อมูล" render-time classifier / wizard Next gate at 0 tagged, `LITE_UX_BATCH2_OK` 5 checks; F-5 manual-repro documented). **Batch 3 ✅ `fcf5b23` 2026-07-03** (F-8: 11 error messages + Thai next-step / annotate Shift-hotkeys ×7 ผ่าน guarded central keydown ⇧T⇧M⇧A⇧H⇧R⇧C⇧U / PM+wizard แปลไทย / NEW empty-state.js overlay ก่อนเปิดไฟล์ / NEW page-scan-badge.js badge ถาวรหน้า scanned+fallback, `LITE_UX_BATCH3_OK` 5/5). เหลือแค่: export entry ซ้ำซ้อน (ต้องเลือก design ทางเข้าเดียว — semi-design) — คิวต่ำสุด | `/bma-ui-scope` per batch | ui (lite) | 🟠 batch 1-3 done, เหลือ export-entry consolidation | — |
...
### bugs 2026-07-06 (user field report, lite — filed via /bma-bug-report, triaged, NEEDS-REPRO-CONFIRM ก่อนลงมือ)

- [x] **BUG-20260706-lite-active-layer-not-following-page** — **BROKEN** (upgraded from FRICTION) — lite — source=bug-report (user) — **✅ FIXED `ba109f0` 2026-07-06** (fallback + commit-guard + hintFlash; `LITE_ACTIVE_LAYER_FOLLOW_OK`; 7 suites green — pending user field re-test)
    - Symptom (repro ยืนยันด้วย screenshot 2026-07-06, `lite/bug/Screenshot 2026-07-06 091956.png`): อยู่หน้า 29 (หลังคา) แผง layer สลับ folder ตามหน้าถูกต้อง แต่ป้าย "วาดที่: ผังบริเวณ · ที่ดิน" + status "วัด: ที่ดิน (ซ่อน)" — **active draw layer ไม่ตามหน้า** และยังวาดลง layer ต่างหน้า/ซ่อนได้เงียบ ๆ → พื้นที่ถูกบันทึกผิดชั้น (data-correctness ไม่ใช่แค่ visual). Hypotheses เดิม (vertMode / CFSS) **ถอนทิ้ง**.
    - **ROOT CAUSE PINNED 2026-07-06 (deep trace, read-only):** (a) `_lsSyncActiveCatToFolder` (`layer-scope.js:449-471`, จาก `e1c6a76`) **ไม่มี fallback branch** — restore เฉพาะเมื่อ folder เคยถูกจำใน session (`mem[folder.id]`); เข้า folder ที่ไม่เคยแวะ → return null → `state.activeCat` (`ui-lite.html:251`) ค้างเป็น layer ของ folder เก่า (wiring ครบทุก nav path ผ่าน `afterPage()→buildPicker()` — ไม่ใช่ path gap); (b) commit path **ไม่มี guard**: `mkObj()` (`ui-lite.html:558` `catId:state.activeCat`) + `finishDraft()` push ลง `PSpage().objects` โดยไม่เช็คว่า layer อยู่ใน folder ของหน้าปัจจุบัน.
    - **Fix plan (minimal, 2 จุด belt-and-suspenders):** (1) `layer-scope.js:465` เพิ่ม else-branch — folder ใหม่ไม่มี memory → default เป็น layer แรกของ folder นั้น (~5 บรรทัด); (2) `mkObj()`/`finishDraft()` เพิ่ม JIT-gate ตาม pattern tag-jit/SCALE-GATE — layer ต่างหน้า → refuse + banner "เลเยอร์ที่เลือกไม่ได้อยู่ในหน้านี้" 1-tap เลือกใหม่. Acceptance: nav เข้า folder ไม่เคยแวะ → `state.activeCat` เป็น layer ของ folder นั้น; force foreign activeCat → `finishDraft()` refuse. Test: extend `lite/tests/test_layer_scope.py` (no-prior-memory case + commit-guard case).
    - Routing: `/bma-lite-dev` · เทสรันตรง `lite/tests/` · risk=low (ไม่แตะ measure-engine/RS/pdfToC/snap) · **BROKEN → ควรถูกหยิบก่อน BUG-20260706-lite-multi-site-page-tag**

- [x] **BUG-20260706-lite-multi-site-page-tag** — FRICTION — lite — source=bug-report (user) — **✅ FIXED `ba109f0` 2026-07-06** (page-aware `_lsGoTo` + rail step-within-folder + "แผ่น i/N"; `LITE_LAYER_SCOPE_MULTI_PAGE_FOLDER_OK`; pending user field re-test)
    - Symptom: ผังบริเวณ (site) มีมากกว่า 1 แผ่น → ในขั้นเลือก/แท็กหน้า เปิดไม่ได้หรือเปิดได้ไม่ครบทุกแผ่น
    - **Triage 2026-07-06 (bma-bug-triager):** data model รองรับหลายหน้า (`PF_site.pages[]`, `_lovsLayerArea` รวมทุกหน้า) แต่: **(1) wizard lock ปลดเร็วเกิน** — `_lwizCheckLiftLock()` (`wiz-auto.js:92-101`) ปลด hard-block ทันทีที่มีหน้า "ใดหน้าหนึ่ง" ถูกแท็ก (ยืนยันโดย `test_wiz_auto.py` sub-check `escAllowedAfterFirstTag`) → แท็กแผ่น site แรกเสร็จ Esc ปิด wizard ได้เลย แผ่นที่ 2 ไม่เคยถูกแท็ก จึง "หาย"; **(2) ทุกหน้า site รวมโฟลเดอร์เดียว `PF_site`** (`page-folder-layers.js:104-157`) + `_seedBaseLayers` (`:244-300`) seed layer "ที่ดิน" ครั้งเดียว → หลายแผ่นแชร์ layer เดียว ดูเหมือน "นับแค่แผ่นเดียว"; **(3) ไม่มีเทสเคส >1 site page เลย** (`test_overview_setup.py` แท็ก site หน้าเดียวทุกเคส — regression blind spot); (4) bulk ⚡ Apply hardcode `tag="floor"` — minor UX.
    - **Repro ยืนยันแล้ว 2026-07-06 (user):** "ติดป้ายครบแล้วแต่ข้อมูลโชว์แค่แผ่นเดียว" → hypothesis (1) wizard lock **ตัดทิ้ง**.
    - **ROOT CAUSE PINNED 2026-07-06 (deep trace, read-only):** folder membership ถูก (`seedPageFolders` `page-folder-layers.js:377` → `PF_site.pages=[2,5]`) และ aggregation ถูก (`objectTuples` `object-agg.js:167-191` เดินทุกหน้า) — ตัวพังคือ **navigation surface ใหม่จาก INV-2026-07-04-001**: `_lsGoTo(folder)` (`layer-scope.js:153-158`) วาร์ปไป `folder.pages[0]` เสมอ และ floor-rail/dropdown/search (`lsRenderRail :171-238`, `_lsRenderSearchResults :390+`) นับ PF_site เป็น 1 stop ไม่ว่ามีกี่หน้า → ไม่มีทางไปแผ่น site ที่ 2 ผ่าน rail/search เลย (ไปได้แค่ thumbnail/next-page ธรรมดา) → ผู้ใช้ไม่ได้วาดแผ่นที่ 2 → Review/report จึงโชว์ข้อมูลแผ่นเดียว (อาการโผล่ที่ report แต่สาเหตุอยู่ที่ nav).
    - **Fix plan (minimal):** (a) `_lsGoTo` page-aware — ถ้า `curPage` อยู่ใน `folder.pages` และมี >1 หน้า ให้ step ไปหน้าถัดไปในโฟลเดอร์ (wrap) แทน reset ไป `pages[0]`; (b) `lsRenderRail` เพิ่ม sub-position indicator "p1/2" เมื่อ folder มีหลายหน้า + ◀/▶ step ภายในโฟลเดอร์ก่อนข้ามโฟลเดอร์; (c) search ได้ฟรีจาก (a). Acceptance: แท็ก site 2 แผ่น → rail/dropdown/search ไปถึงทั้ง 2 แผ่น + วัดได้ทั้งคู่ + review `sitePages` ครบ. Test: sub-check ใหม่ใน `lite/tests/test_layer_scope.py` (`LITE_LAYER_SCOPE_MULTI_PAGE_FOLDER_OK`, pageTags={2:'site',5:'site'}, `_lsGoTo` ×2 บนโฟลเดอร์เดิมต้องเลื่อนหน้า) + เคส 2-site-page ใน `test_overview_setup.py`.
    - Routing: lite-only — `/bma-lite-dev` · เทสรันตรง `lite/tests/` · risk=low · **status: READY-TO-FIX (root cause + fix plan ครบ รอ user สั่งแก้)**
...
### findings 2026-08-10 (user field report + invent checkpoint)

- [x] **BROKEN(UX): เปิด Page Manager ไม่ได้ — wiz-auto lock กลืน ⇧F12 + เมนูอยู่ใต้ wizard overlay** — `BUG-20260810-lite-pagemgr-blocked` — **✅ CLOSED STRUCTURALLY `fb9b2af`** (WIZ-UNLOCK ปลด auto-open+hard-lock — ไม่มีตัวกลืนคีย์อีก; PM-GUARD `c88a379` เพิ่ม no-PDF hint แทน silent return) — user field report 2026-08-10 ("เปิด pagemanager ไม่ได้"). Root cause CONFIRMED by Playwright repro (scratchpad repro_pagemgr.py): (1) หลังเปิด PDF, wizard auto-open + capture keydown lock (`wiz-auto.js:60-79`, `stopImmediatePropagation` จนกว่าจะ tag ≥1 หน้า) กลืน ⇧F12; เมนู `mi-pagemgr` กดไม่ถึงเพราะ `#ov` ทับ; direct call `pmOpenManager()` เปิดได้ปกติ = ตัว PM ไม่พัง, ไม่ใช่ regression จาก 878effd; (2) ก่อนเปิด PDF, guard `pmOpenManager` (`page-manager-ui.js:492`) return เงียบไม่มีข้อความ. Repro matrix: no-PDF→เงียบ · post-PDF+wizard→⇧F12 ตาย · tag 1 หน้า→⇧F12 ใช้ได้. Fix direction (เล็ก, 2 จุด): allowlist ⇧F12 ใน wiz-auto capture lock (เหตุผล workflow: ผู้ใช้อาจต้องลบ/เรียงหน้าขยะ*ก่อน* tag) + `pmOpenManager` แจ้ง hint แทน silent return เมื่อ `!caseId||!pageMgr`. Workaround แจ้ง user แล้ว: tag 1 หน้าก่อน. | `/bma-lite-dev` | lite / wizard-lock / page-manager

- [x] **page-manager-redesign** — **✅ SHIPPED 2026-08-10** `c88a379`+`b0a13bf`+`fb9b2af` — (**GO แนวทาง D — user 2026-08-10**, build 3 slices ผ่าน /bma-lite-dev: PM-GUARD → TAG-JIT-FIX → WIZ-UNLOCK) — full doc `docs/invent/page-manager-redesign.md` (2026-08-10, `/lite-invent` 7 เฟสเต็ม, ต่อยอดจาก BUG-20260810 + user ขอวิจัยใหม่ทั้งตัว). Research verdict PRIOR_ART_PARTIAL (incumbent ทั้ง 5 รายไม่มีใครใช้ dismissable modal กับงานจัดการหน้า; NN/g ห้าม click-outside-close บนจอถือ unsaved state). Diverge 5 ทาง — **ชนะ: D (25) ปลด wizard auto-open/hard-lock → JIT tag banner + เสริมเกราะ PM** (ปิด BUG-20260810 เชิงโครงสร้าง, net ~0 ต่อเพดาน ui-lite) · fallback A (24, เกราะอย่างเดียว ~45 บรรทัด) · หมายเหตุ: mockup ศูนย์หน้า = แนวทาง C ได้ 20 (blast radius ใหญ่เกิน — เก็บเป็นวิสัยทัศน์ระยะยาว). **Spike ALL PASS 14/14** (`lite/sandbox/invent-page-manager-redesign/spike.html`, รัน headless แล้ว) รวมเคสจำลองอุบัติเหตุ user ตรงๆ. คำถาม checkpoint: ยอมรับการปลด wizard บังคับหรือไม่ (นโยบาย UX ไม่ใช่เทคนิค). **HALTED — รอ GO / NOGO / RESHAPE**
...
- [ ] **XLSX ไม่มีคอลัมน์ความยาวเส้น** — `queued` `S` — builder finding ระหว่าง CURVE-LEN: `buildExportData()` แถว line/path/ref มีแต่ `area:null` ไม่มี length และ `/export-xlsx` ใน server_lite เขียน 6 คอลัมน์ตายตัวอ่านแค่ area/count → ความยาว (รวม arc) โชว์บนจอ/label แต่**ไม่ไปถึง Excel** — fix ต้องแตะ server_lite.py (นอก allowlist ของ sprint ที่แล้ว จึง file แยก): เพิ่มคอลัมน์ Length (m) + ฝั่ง client ส่ง length ใน row (ใช้ lineLenWithArcs) | `/bma-lite-dev` | lite / export

### sprint cards CURVE-2026-08-11 (วัดความยาวเส้นโค้ง/freehand ใน lite — user GO "ทำให้ใช้ได้ใน lite")

- [x] **CURVE-LEN: arc ในเครื่องมือระยะต่อเนื่อง (⇧D)** — **✅ SHIPPED `37aabef`** (RED→GREEN, ยาว arc ตรง closed-form 0% err) — — ฐาน: arc kernel LCURVE ที่ polygon ใช้อยู่ (`draw-arc.js` + measure-engine arc fns, PRIOR_ART_MATURE → ข้าม invent ตามกฎ; ต้นเรื่อง: user 2026-08-11 "วัดเส้นโค้ง/freehand เป็นวิธีวัด"). ท่าเดียวกับ polygon: คลิกจุดผ่านโค้งระหว่างวาด path → ความยาว = r×|sweep| จริง (helper ใหม่ข้าง engine ห้ามแตะ vendored) · **SCOPE ritual:** (a) แตะ I4 (edges บน line object ต้อง save/load additive — ระวัง: buildPageStore เดิมทิ้ง edges ของ line!) + I5 (label/HUD/XLSX/overlay ต้องตรงกัน — overlay ส่ง pts ที่ flatten arc แล้ว) — เพิ่ม fixture เข้าเทสต์ persist + export; ไม่แตะ I2 (line ไม่เข้า area rollup) (b) ไม่มี object kind ใหม่ (line เดิม + edges additive) · guard: `LITE_CURVE_LEN_OK` RED ก่อน | `/bma-lite-dev` | lite / measure-length

- [x] **FREEHAND-LEN: Alt ค้าง+ลาก บน ⇧D** — **✅ SHIPPED `37aabef`** (RED→GREEN, err 1.3%, RDP 86→9 จุด) — — ฐาน: **พอร์ตดีไซน์ที่ ship แล้ว** INV-2026-05-17-001 (proto `023b988`, Approach D ที่ user เลือกเอง: Alt-hold = streaming sub-mode, distance-bin ≥6px, ปล่อย=กลับโหมดคลิก ผสมได้, จบ→`rdpSimplify` ~25 LOC พอร์ตจาก proto) → ความยาว polyline · SCOPE: I4 (optional `obj.freeform` additive) + I5; snap/CL bypass ระหว่างลาก (ตามแบบเดิม) · guard: `LITE_FREEHAND_LEN_OK` RED ก่อน · ทำหลัง CURVE-LEN | `/bma-lite-dev` | lite / measure-length
...
- [x] **INFRA-CI: GitHub Actions — ด่านทั้งหมดรันเองทุก push** — **✅ SHIPPED 2026-08-10** — `.github/workflows/ci.yml`. เหตุผล: ด่าน 6 ตัว + suite ที่สร้างมาทั้งหมด "มีอยู่จริงเฉพาะตอนมีคนสั่งรัน" — ช่องว่างอันดับ 1 เทียบกับมาตรฐานอุตสาหกรรม (จากบทสนทนาแนวปฏิบัติ Anthropic/OpenAI). โครง 3 job: (1) `truth-gate` ubuntu — `check_executable_truth --verbose` (fetch-depth 0 เพราะ ships-commits ต้องมี git history); (2) `fast-tests` ubuntu — tier t0 (parity+PBT ผ่าน Node) + t1 (server endpoints) ทุก push, เร็ว ไม่ต้อง browser; (3) `full-suite` windows-latest (ตรงกับเครื่อง dev ที่เขียนเทสต์) — Playwright เต็มชุด เฉพาะ manual dispatch + nightly cron, ตั้ง `continue-on-error` ชั่วคราวเพราะ `test_closing_dup_strip.py` แดง pre-existing (ถ้าไม่ตั้ง badge จะแดงถาวรจนคนเลิกมอง)
    - **ค้าง/ข้อจำกัดที่ต้องรู้:** (a) cron nightly จะยิงจาก **default branch เท่านั้น** — ตอนนี้ default คือ `main` (สายเก่าที่ค้างตั้งแต่ พ.ค.) ส่วนงานจริงอยู่ `main-v2-2026-05-19` → nightly ยังไม่ทำงานจนกว่าจะย้าย default branch (ผูกกับคำถามค้าง "ทับ main เลยไหม"); (b) job บน ubuntu ยังไม่เคยพิสูจน์ว่าเทสต์ที่เขียนบน Windows รันผ่านบน Linux — รอผลรันจริงครั้งแรก; (c) `gh` ในเครื่องยังไม่ auth ผมจึงตรวจผลรันแทนไม่ได้ ต้องดูแท็บ Actions เอง
    - Follow-up: ปลด `continue-on-error` เมื่อ `test_closing_dup_strip.py` ถูกแก้
...
- [ ] **Track AI อ่านแบบแปลน — AI เสนอ 80% แรก คนยืนยัน 20% สุดท้าย** — `invent-queued` — from /idea 2026-08-10 22:01 (full body IDEAS.md `2026-08-10-22-01`)
    ...
    - Scope skill: `/lite-invent` (ขั้น 1 eval-first เป็น invent pass แรก — ground truth จาก .bmaplan ที่วัดมือแล้ว)

### sprint cards SHELL-2026-08-10 (main-screen UI — mockup rev.3 approved-in-principle, `needs-GO`)

- [x] **SHELL-STATUS: status bar ล่าง** — **✅ SHIPPED 2026-08-10** `2b1887f` — (**GO — user 2026-08-10**, คิวหลัง PM-redesign build เสร็จ) — mockup `lite/sandbox/mockup-main-shell.html` (rev.3). โมดูลใหม่ `static/js/status-bar.js` (~200 บรรทัด, self-inject ห้ามแตะ ui-lite.html เกิน 1-2 บรรทัด script tag) แสดง 7 ช่อง: หน้า/ชั้น (คลิก=เมนู Page) · ⚖ scale ✓/✗ · เครื่องมือ · วาดเป็นประเภทไหน · 🧲 snap (แทน indicator ที่หายไปกับบั๊กเมนู flyout) · ● dirty · สุทธิชั้นนี้ · ⇧F focus ซ่อนได้ · **SCOPE ritual ตอบแล้ว:** (a) ช่อง "สุทธิชั้นนี้" = consumer ใหม่ของระบบรวมยอด → **ต้องลงทะเบียนใน INVARIANTS I2 consumer table + เพิ่ม fixture เข้า test_summary_arc_parity ภายใน sprint เดียวกัน — ไม่มีข้อยกเว้น** (b) ไม่มี object kind ใหม่ · guard marker: `LITE_STATUS_BAR_OK` (RED ก่อน) · เหตุผลข้าม invent: PRIOR_ART_MATURE (status bar 7 ช่องเป็น contract เดิมของ proto ที่ lite ตัดทิ้ง — เอาคืนแบบ read-only) | `/bma-lite-dev` | lite / shell
- [x] **SHELL-FLOAT: panel เลเยอร์ลอยแบบ Photoshop** — **✅ SHIPPED 2026-08-10** `2b1887f` — (**GO — user 2026-08-10**, ทำหลัง SHELL-STATUS) — ทำหลัง SHELL-STATUS. โมดูลใหม่ `static/js/float-panel.js` (~120 บรรทัด) **ครอบ #picker เดิมโดยไม่แตะเนื้อใน**: ลาก title bar (⠿) + clamp ขอบจอ + พับ ▾ + ปิด ✕ + จำตำแหน่ง localStorage (`bmaLite.floatPanel.v1` — precedent: proto `bmaPlan.widgetPlacement.v1`) + ปุ่ม reset ตำแหน่งในเมนู View กัน panel หลุดจอ · SCOPE: ไม่แตะ invariant ใด (UI ล้วน) · ความเสี่ยงที่รู้: z-index ชน overlay (มี audit list จากรีวิว 2026-08-10) · guard marker: `LITE_FLOAT_PANEL_OK` · ฐานงานวิจัย: `docs/invent/fullscreen-canvas-ui.md` (GO 2026-05-19) — canvas-first + Bluebeam floating-toolbar pattern | `/bma-lite-dev` | lite / shell

### sprint SNAP-2026-07-04 (Fable snap review → user delegated selection 2026-07-04)

- [x] **SNAP-2026-07-04: snap-polish (lite)** — `✅ SHIPPED 2026-07-04: cd6a960 (slice 1 extract snap-engine.js + intersection cache, ui-lite 1192→1158) · 23f3914 (slice 2 lock+snap compose, click==preview, +orthoOn indicator side-fix) · 42a0767 (slice 3 nearest toggle 5-type symmetry)` — Fable reviewed the snap system (object-snap computeSnap ui-lite:635 + centerline vendored + ortho) and selected per principles: **slice 1** extract snap core → `static/js/snap-engine.js` (ui-lite at 1192/1200) + per-page seg/intersection cache w/ mutation invalidation (kills the O(n²)-per-mousemove scan, behavior identical); **slice 2** angle-lock + snap COMPOSE (locked ray ∩ segments + endpoint projections on ray, nearest-along-ray wins; fallback pure lock — CAD "apparent intersection" semantics; today lock discards snap entirely, liveTarget ui-lite:669); **slice 3** "nearest-on-edge" gets a toggle like the other 4 types (symmetry fix, snap-types.js). **CUT (deliberate):** unifying centerline into the same candidate pipeline — deferred until INV-2026-05-25-001 field retest closes (touching CL glue now would confound the user's pending retest); wall-trace assist → /idea (invent-queued below). Forbidden: measure-engine/pdfToC/RS untouched; extraction verbatim-first with identical-output test. | `/bma-lite-dev` | lite / snap-ux |

### findings 2026-07-04 (evidence-photographer pass, post report-truth)

- [x] **BROKEN: PDF with native /Rotate opens sideways (PDF.js path discards intrinsic rotation)** — `BUG-20260704-lite-native-rotate` — **✅ FIXED `fbe28fb` same day** (2 one-line fixes: natVp + thetaTotal include `cp.rotate`; repro-first fixture 6 pre-fix fails → 24/24; overlay/pagerot registration ≤0.5px + parity green; awaiting user re-test). — user field report 2026-07-04 ("pdf ที่เปิดถึงกลับข้าง ต้องหมุน 90"). Root cause CONFIRMED: `page-renderer.js:600-606` passes `rotation: (V.rot+pgRot)` to `getViewport()` — explicit rotation REPLACES the page's intrinsic /Rotate in PDF.js (the comment at L596 asserting "intrinsic via default rotation=0" is factually wrong about the API). Raster-fallback path is correct (PyMuPDF get_pixmap applies native, then prerotate(user)) → two render paths disagree. Regression from GO-20260703 streaming migration; proto unaffected (server-side render). Fix: fold `cp.rotate` (native) into thetaTotal everywhere rotation composes (viewport L606, natVp L550 usage, fit() dim-swap, overlay ctx transform) — registration + parity tests mandatory; never touch measure-engine/pdfToC/cToPdf themselves. Needs native-/Rotate fixture PDF in tests.

- [x] **COSMETIC: report shows 0.00 ตร.ม. for unscaled-page areas** — **✅ FIXED `a5044aa`** (SCALE-GATE: JIT gate ชั้นสอง — วัดหน้าไม่มี scale โดน refuse + banner ตั้ง scale 1 แตะ + re-arm เครื่องมือเดิม; รายงานแสดง "—" แทน 0.00, subtotal/net ข้าม null; count exempt; help-system layers 2/3 ตัดตามคำตัดสิน user) — filed 2026-07-04 from screenshot-proof run. Root cause traced: `buildReportPayload()` does `a=(a==null?0:a)` (`export-annotate.js`) while `polyMetricsAnyShape` returns `null` area when the page has no scale (`measure-engine.js:41`) → "ยังไม่ตั้ง scale" แสดงเป็น 0 แยกไม่ออกจากพื้นที่ศูนย์จริง อาจหลอกคนตรวจรายงานหน้าที่ลืม calibrate. Fix direction: แสดง "—"/badge "ยังไม่ตั้งมาตราส่วน" แทน 0 (display เท่านั้น ห้ามแตะ measure-engine). เข้าเกณฑ์ JIT-gate ธีมเดียวกับ HT-7 ด้วย — พิจารณาให้ per-page gate เตือนเรื่อง scale พร้อมกัน

### ideas 2026-07-04

- [ ] **Raster wall-trace assist (คลิกต้น-ปลาย ระบบไล่เส้นผนังให้)** — `invent-queued` — from Fable snap review 2026-07-04, user-delegated selection; full body IDEAS.md `2026-07-04-21-45`
    - Gist: ต่อยอด skeleton primitives ของ centerline-snap (Otsu→thinning→path-follow) จาก snap จุดเดียวเป็น trace ทั้งช่วง
    - Tags: bma-plan, lite, measure, snap, raster, geometry, p-med
    - Depends-on: INV-2026-05-25-001 ปิดก่อน (primitive ชุดเดียวกัน, Section A drift-locked)
    - Scope skill: `/lite-invent` (research: AutoCAD raster design / magnetic lasso / live-wire segmentation prior art)

- [x] **Report truth: single source for every export number** — `✅ SHIPPED 2026-07-04, 5 slices same day` — A `bb5090f` (XLSX+Σ→tuple stream, excluded ความหมายเดียว = A-4/B-6) · B `6ba7ea3` (grid ครบทุกหน้า+sign = B-2/B-3) · C `fc63e72` (grid โหมดเดียวพิมพ์จริง ปลด classic = S-1ก; reportVars re-homed) · D `8362c3f` (ประตู 3 ป้ายตรง + label ไทย = S-6/S-12) · E `52725a1` (ภาคผนวกรูปแปลน+overlay — user เคาะเพิ่มที่ checkpoint) — from /idea 2026-07-04 (user: "ใช้ความคิดเหนของ fable")
    - Source: user 2026-07-04, "ใช้ความคิดเหนของ fable"; full context in IDEAS.md `2026-07-04-16-01`
    - Gist: หลักการ "เลขหนึ่งตัว มีที่เกิดที่เดียว" — (1) report-truth sprint: reroute XLSX `buildExportData` เข้า tuple stream (= A-4) + excluded ความหมายเดียว + S-1 ทาง ก (grid ครบทุกหน้า พิมพ์จาก grid ปลด classic) → ปิด B-2/B-3/B-6/กับดักพิมพ์ในก้อนเดียว; (2) ประตู export 5→3 ป้ายตรง (= S-6); (3) PDF overlay label ชื่อไทย (= S-12)
    - Tags: bma-plan, lite, report, export, p-high
    - Note: maps to `REVIEW_LITE_LAYER_REPORT_20260704.md` ledger S-1(ก)+A-4+S-6+S-12 — เคาะ per-item GO ตอนหยิบทำ (กติกา ledger); bugs B-2/B-6 ยืนยันด้วยตาแล้ว (lite-report.html:264 `pages[0]`, export-annotate.js:11 ไม่กรอง excluded)
    - Forbidden-surface profile: คาดว่า clean (report/export layer เท่านั้น ไม่แตะ measure math) — ยืนยันตอน scope

- [x] **Page tagging: just-in-time + range-tag + verify grid + fearless retag** — `invent-done-go (→ INV-2026-07-04-002)` — full pipeline 2026-07-04: research PRIOR_ART_PARTIAL (reclassify-after-draw = greenfield, remap math in-repo), 5 approaches, spike C+minimal-A 3/3, opus CONFIRM-WITH-CONCERNS, user GO — from /idea 2026-07-04 (user: "ทำที่ fable คิด")
    - Source: user 2026-07-04, "ทำที่ fable คิด"; full context of the referenced opinion in IDEAS.md `2026-07-04-13-21`
    - Gist: keep human-tags-as-ground-truth; demote wizard from mandatory gate to optional tool — (1) tag-at-first-measure, (2) range-tag runs ("หน้า 11–16 = แปลนชั้น เริ่มชั้น 1"), (3) thumbnail grid grouped by tag for visual verification, (4) retag remaps folder+objects safely + undoable
...
- [x] **Lite layer menu UI fix** — `invent-done-go (→ INV-2026-07-04-001)` — full pipeline 2026-07-04: reframed to 100-floor scalability, research PRIOR_ART_PARTIAL (incumbents separate floor⊥layer axes), 5 approaches, spike B passed 9/9 (3 cases × 3 proportion presets), opus review CONFIRM-WITH-CONCERNS, user GO + picked P2 260px — from /idea 2026-07-04 (user typed while working in lite)
    - Source: user 2026-07-04, "ใน lite  แก้ไข ui ในส่วนของ เมนู layer"
...
- [~] **LFOC-1e** — `absorbed → INV-2026-07-04-001` (2026-07-04) — Click layer/obj in panel → warp to that page. Now acceptance criterion #1 (bidirectional PDF-page binding) of the context-scoped layer panel sprint; do not build separately.
