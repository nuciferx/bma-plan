...
| INV-2026-05-18-001b | **Page Setup Redesign Part B — Floor sub-types for plan tag** — Scope narrowed to user's stated requirement (floor sub-types). New per-page schema `pageFloorKind ∈ {basement|normal|mechanical|rooftop|custom}` + `pageFloorNum` (additive, optional). New helpers `setPageFloorKind`/`setPageFloorNum`. `autoNamePage` floor-aware for `tag=plan`: basement N → "ชั้นใต้ดิน N", normal N → "ชั้น N", mechanical → "ชั้นห้องเครื่อง", rooftop → "ชั้นดาดฟ้า", custom → leaves `pageNames[n]` for user. `setPageTag` clears floor fields on tag change away from plan. Save/load round-trip via `_makeProjBlob` + `applyLoadedProject`. Floor-kind picker in page-card (visible when tag=plan), numeric input for basement/normal. Token-engine refactor deferred — current prefix inputs preserved for backward compat. `PHASE_INV_PAGE_SETUP_B_OK` 9/9 PASS (7 contract + 2 bonuses: save/load round-trip, tag-change clear). Zero forbidden-surface. | `/bma-ui-scope` | UI / template + schema additive | ✅ done | INV-2026-05-18-001a |
