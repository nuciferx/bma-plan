...
- **(3) Real-raster threshold robustness** — current Otsu binarisation works on synthetic test + first user PDF. If contrast-faded or low-DPI scans misfire, swap to adaptive Sauvola/Niblack. Wait for user reports before committing to this sprint — do not run preemptively.

- **(4) Sharp corner < 60° PCA fallback** — PCA edge-line fit from 5 samples can be unstable on very short polygon edges (< 20px). Add step-1-only fallback when refine confidence is low (computed from PCA residual). Small, safe, additive.
