...
Date: 2026-05-26 (updated: BUG-20260526-lite-stale-pf-folder-cleanup SHIPPED. LITE-PROBE-DBLCLICK-REWRITE queued as next test-infra sprint.)

## Immediate Next

**BUG-20260526-lite-stale-pf-folder-cleanup SHIPPED. PF folder cleanup is now correct. LITE-PROBE-DBLCLICK-REWRITE queued (test-infra).**

- **(1, QUEUED) LITE-PROBE-DBLCLICK-REWRITE** (test-infra, medium priority): Rewrite `LITE-BUG-DBLCLICK-OVER-POP` regression probe in `.claude/skills/bma-simulate/regression_probes.json` from `mouse_sequence` to `evaluate`-only. Current approach uses 4 real Playwright `page.mouse.click` + 1 `page.mouse.dblclick` — vulnerable to any UI workflow change that puts a modal/wizard in front of the canvas (e.g. the Page Setup wizard auto-open from BUG-20260526-lite-force-setup). New approach: directly inject `state.draft` points via `evaluate`, then dispatch a synthetic `dblclick` event on `cv`. Makes probe robust against future UI changes. Current setup_js workaround (`_lwizAutoLiftLock()` + clear `ov.show` class) is partial and creates drift risk. No runtime code changes needed — probe JSON only.

- **(2) INV-2026-05-25-001 — centerline-snap real-PDF robustness** — first iterate `3d4a53e` shipped (ROI retry ladder + flash dot). Open work: (a) user re-tests `SCR_ผังต่อโฉนด.pdf` with new feedback, (b) add real-raster fixture PDF to `lite/tests/fixtures/`, (c) decide Otsu → Sauvola/Niblack swap for low-contrast scans, (d) mirror to proto after lite stabilizes. Status: WAITING ON USER FEEDBACK.
