...
Date: 2026-06-05 (updated: SLICE report-edit-1 SHIPPED. Editable lite report live behind #re-toggle dev gate. Next sprint = production wire-up + print CSS re-validation.)

## Immediate Next

**SLICE report-edit-1 SHIPPED. Editable report is behind `#re-toggle` dev gate. Three follow-on sprints identified.**

- **(1, QUEUED) report-edit production wire-up** — remove the `#re-toggle` dev gate and wire `report-edit.js` into the default report open path in `lite/lite-report.html`. Confirm zero regression on existing LITE_REPORT_OK (17/17) + LITE_REPORT_EDIT_OK (7/7). No forbidden surface touched; lite-only.

- **(2) report-edit print-to-PDF CSS re-validation** — the jss grid replaces the contenteditable report wholesale; `@page A4 landscape` + `@media print page-break-after:always` rules applied to the old HTML `<table>` may not translate to the jss grid renderer without adjustment. Sprint = open browser print-preview, measure what breaks, patch CSS only. Additive; no JS logic change.

- **(3) report-edit persistence → .bmaplan** — promote localStorage v1 keyed by payload hash to an additive `.bmaplan reportEditState` field (invent Approach E; semantic ids proven to serialize cleanly by the PERSIST eval case). Requires careful additive-only schema work; run `/bma-check-forbidden` on `.bmaplan` schema before starting.

- **(4, QUEUED) LITE-PROBE-DBLCLICK-REWRITE** (test-infra, medium priority): Rewrite `LITE-BUG-DBLCLICK-OVER-POP` regression probe in `.claude/skills/bma-simulate/regression_probes.json` from `mouse_sequence` to `evaluate`-only. Current approach uses 4 real Playwright `page.mouse.click` + 1 `page.mouse.dblclick` — vulnerable to any UI workflow change that puts a modal/wizard in front of the canvas. New approach: directly inject `state.draft` points via `evaluate`, then dispatch a synthetic `dblclick` event on `cv`. No runtime code changes needed — probe JSON only.

- **(5) INV-2026-05-25-001 — centerline-snap real-PDF robustness** — first iterate `3d4a53e` shipped (ROI retry ladder + flash dot). Open work: (a) user re-tests `SCR_ผังต่อโฉนด.pdf` with new feedback, (b) add real-raster fixture PDF to `lite/tests/fixtures/`, (c) decide Otsu → Sauvola/Niblack swap for low-contrast scans, (d) mirror to proto after lite stabilizes. Status: WAITING ON USER FEEDBACK.

- **(3) Vector PDF route (Approach E from diverge)** — extract stroke-width from `extract_snaps_typed()` server response (additive field), client offset by w/2 when `snap.t==='nl'`. Allows centerline snap via vector geometry. Separate sprint; run `/bma-measure-scope` first.

- **(4) Real-raster threshold robustness** — swap Otsu → Sauvola/Niblack adaptive threshold for low-contrast scans. Wait for user reports before committing.

- **(5) Sharp corner < 60° PCA fallback** — PCA edge-line fit from 5 samples can be unstable on very short polygon edges. Add step-1-only fallback when refine confidence is low. Small, safe, additive.
