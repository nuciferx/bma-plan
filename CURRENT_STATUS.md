...
Date: 2026-07-02 (BUG-20260702-lite-cfss-summary SHIPPED. CFSS shared-shape instances were skipped by every area rollup (silent total loss) and crashed XLSX/annotated-PDF export outright (catOf called before kind guard). Fixed by passing catId/semanticTag into addMaster at promote time + new rollupAreaM2/rollupCatId dispatch helpers. LITE_SUMMARY_CFSS_OK NEW, proven RED→GREEN. Zero proto/ edits. Both bugs from the 2026-07-02 measurement-accuracy audit now shipped — bug 1 = BUG-20260702-lite-arc-summary (earlier today). Previous ship before the audit: SLICE report-edit-1 SHIPPED 2026-06-05.)
...
Older sprints (LITE-0, HT-ACC series and earlier) — see [docs/status/NEXT_ACTIONS.md](docs/status/NEXT_ACTIONS.md) "Recently Done" and archived logs.
